...
| 核心框架 | Go `net/http` + `database/sql` | 模块化单体、显式依赖、低运行时开销 |
...
| ADR | 决策 |
|---|---|
| ADR-001 | 使用 Go 模块化单体，不使用 TypeScript/Java 后端 |
...
## 23. PLAN-007 完工质量与售后闭环技术落地

完工后不再用单一状态表达所有语义。`work_order` 同时维护 `visit_status`、`customer_acceptance_status`、`internal_review_status`、`closure_status` 和 `completion_outcome`；完工提交、客户验收、客户评价、质检审核、总监审核、客服确认分别写入独立事实表，并追加 `work_order_event`，从而支持并行写入、幂等约束和完整时间线。

核心接口：

- `POST /api/v1/mini/work-orders/{id}/acceptance`：人工验收；
- `POST /api/v1/mini/work-orders/{id}/rating`：一次性评价；
- `POST /api/v1/admin/work-orders/{id}/internal-review?level=QA|DIRECTOR`：两级审核；
- `POST /api/v1/admin/work-orders/{id}/customer-service-confirmation`：客服确认是否二次上门；
- `GET /api/v1/admin/work-orders/{id}/timeline`：完整流程时间线；
- `GET /api/v1/admin/work-orders?completionOutcome=CUSTOMER_CONFIRMED_NO_SECOND_VISIT`：筛选客户确认无需二次上门的工单。

PostgreSQL 迁移 `000005_parallel_acceptance_review` 增加完工提交、验收、评价、内部审核、客服确认和事件表，并为评价、同一完工提交的验收及客服确认设置唯一约束，保证重复请求不会产生重复事实。自动验收由到期任务按 `auto_accept_due_at` 扫描，写入 `source=AUTO`，不覆盖人工验收结果。

...
Page({
  ...
  async load() {
    try { this.setData({ loading: true, error: '' }); const order = await getWorkOrder(this.data.orderId); this.setData({ order: { ...order, statusText: workOrderStatusLabel(order.status) } }) }
  ...
  async command(e: WechatMiniprogram.TouchEvent) {
    ...
    let reason = ''
    if (command === 'REJECT') {
      const result = await new Promise<WechatMiniprogram.ShowModalSuccessCallbackResult>(resolve => wx.showModal({ title: '拒绝接单', editable: true, placeholderText: '请填写拒单原因', success: resolve }))
      if (!result.confirm || !result.content?.trim()) return
      reason = result.content.trim()
    } else {
      const result = await new Promise<WechatMiniprogram.ShowModalSuccessCallbackResult>(resolve => wx.showModal({ title: '确认操作', content: `确认执行“${command}”吗？`, success: resolve }))
      if (!result.confirm) return
    }
    try { await commandWorkOrder(this.data.order.id, command, this.data.order.version, reason); wx.showToast({ title: '操作成功', icon: 'success' }); await this.load() }

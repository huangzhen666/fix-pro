  ...
  return <Card>
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Table rowKey="id" loading={orders.isLoading} dataSource={orders.data?.items} pagination={{ current: page, total: orders.data?.total, pageSize: 20, showSizeChanger: false, onChange: p => setPage(p) }} columns={[
        ...
        { title: '状态', dataIndex: 'status', render: (v: string, row: any) => <Space><Tag color="blue">{adminWorkOrderStatusLabel(v, row.customerAcceptanceStatus)}</Tag>{row.completionOutcome === 'CUSTOMER_CONFIRMED_NO_SECOND_VISIT' ? <Tag color="green">客户确认无需二次上门</Tag> : null}</Space> },

...
export interface SkuSummary { id: string; skuCode: string; name: string; categoryName: string; basePrice: number; unit: string; status: string; publishedVersion?: number; updatedAt: string }
export interface SkuDetail { id: string; categoryId: string; skuCode: string; name: string; description?: string; serviceScope: string; exclusions: string; warrantyDescription: string; priceMode: string; basePrice: number; unit: string; coverMediaId?: string; galleryMediaIds: string[]; status: string; version: number; publishedVersion?: number }
export interface SkuWrite { categoryId: string; skuCode: string; name: string; description?: string; serviceScope: string; exclusions: string; warrantyDescription: string; priceMode: string; basePrice: number; unit: string; coverMediaId: string; galleryMediaIds: string[]; version: number }
...
export const listSkus = (keyword = '', page = 1) => apiRequest<PageResult<SkuSummary>>(`/api/v1/admin/catalog/skus?page=${page}&pageSize=20&keyword=${encodeURIComponent(keyword)}`)

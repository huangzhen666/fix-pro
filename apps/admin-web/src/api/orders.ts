...
export interface OrderSummary { id: string; orderNo: string; status: string; cancelReason?: string; contactName: string; contactMobile: string; totalAmount: number; itemCount: number; createdAt: string; version: number; appointmentAt?: string; appointmentSlot?: string }
...
export const rejectOrder = (id: string, version: number, reason: string) => apiRequest<{orderId: string; orderStatus: string; cancelReason: string}>(`/api/v1/admin/orders/${id}/reject`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ version, reason }) })

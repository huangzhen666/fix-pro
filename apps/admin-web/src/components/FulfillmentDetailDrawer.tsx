...
import { Button, Drawer, Descriptions, Divider, Empty, Image, Input, Space, Spin, Tag, Timeline, Typography } from 'antd'
...
import { adminWorkOrderStatusLabel, enumLabel } from '../utils/enums'
...
const evidenceStageLabels: Record<string, string> = { BEFORE: '施工前', DURING: '施工中', AFTER: '施工后' }
...
export function FulfillmentDetailDrawer({ open, workOrderId, onClose, review }: { open: boolean; workOrderId?: string; onClose: () => void; review?: FulfillmentReviewOptions }) {
  ...
  const displayedWorkOrderStatus = work.data ? adminWorkOrderStatusLabel(work.data.status, work.data.customerAcceptanceStatus) : undefined
  const footer = review ? <Space style={{ width: '100%', justifyContent: 'flex-end' }}><Button onClick={onClose}>关闭</Button><Button danger loading={review.loading} onClick={() => review.onSubmit('REJECT')}>打回</Button><Button type="primary" loading={review.loading} onClick={() => review.onSubmit('APPROVE')}>通过</Button></Space> : undefined
  return <Drawer title={work.data ? `${review ? reviewLevelLabels[review.level] : '履约详情'} · ${work.data.workOrderNo}` : review ? reviewLevelLabels[review.level] : '履约详情'} open={open} onClose={onClose} width={1200} footer={footer}>
    {work.isLoading ? <Spin /> : !work.data ? <Empty description="履约详情加载失败" /> : <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Typography.Title level={5}>订单与客户信息</Typography.Title>{order.isLoading ? <Spin /> : order.isError ? <Typography.Text type="danger">订单详情加载失败，请检查后端服务</Typography.Text> : order.data ? <OrderDetailContent data={order.data} statusOverride={displayedWorkOrderStatus === '已完成待审核' ? displayedWorkOrderStatus : undefined} /> : <Empty description="暂无订单详情" />}

...
export function request<T>(options: WechatMiniprogram.RequestOption): Promise<T> {
  ...
  return new Promise((resolve, reject) => {
    wx.request<ApiResponse<T>>({
      ...
      fail(error) {
        reject(new Error(`请求失败：${error.errMsg || '网络不可用'}（API: ${getApiBaseUrl()}）`))
      },
    })
  })
}
...
export function download(path: string): Promise<string> {
  const token = wx.getStorageSync<string>('fixpro.accessToken')
  return new Promise((resolve, reject) => wx.downloadFile({ url: `${getApiBaseUrl()}${path}`, header: token ? { Authorization: `Bearer ${token}` } : {}, success: response => response.statusCode >= 200 && response.statusCode < 300 ? resolve(response.tempFilePath) : reject(new Error(`图片加载失败 (${response.statusCode})`)), fail: reject }))
}
